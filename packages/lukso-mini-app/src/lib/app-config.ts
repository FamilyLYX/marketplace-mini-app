...
import { lukso, luksoTestnet, xdc, xdcTestnet } from "viem/chains";
...
const useReadClient = () => {
  ...
  return createPublicClient({
    chain: connectedChain ?? (isTestnet ? luksoTestnet : lukso),
    transport: http(connectedChain?.rpcUrls.default.http[0]),
  });
};
export { appConfig, readClient, useReadClient, isTestnet };
